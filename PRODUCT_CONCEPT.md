...
4. **Zahlungsinfrastruktur (Block H)** – bewusst zurückgestellt. **P3.2 Paywall-UI:** Free = alle Lektionen + lokaler Lab-Coach; Pro = Live-KI-Bewertung (BYOK remote) + Zertifikat-Export. Anzeige-Preise: **€9.99/Monat** oder **€59.99/Jahr** (`lib/proPricing.ts`, Screen `/paywall`). CTA schaltet bis Block H lokal Pro frei (ehrliche Footnote); echtes Store-Billing folgt später.
