# HAPTICS.md
### StructAI – Haptics Map v1.2 (verbindlich). Cursor MUSS diese Zuordnung nutzen, keine eigene Erfindung.
...
**Kalibrierung v1.2 (Expo Go, physisches Gerät):** Impact-only (`Light`/`Medium` ohne Notification) blieb beim Antworten unfühlbar. Spürbar war erst `Success/Warning Notification` + `Medium`/`Heavy` Impact (Lektionsabschluss). v1.2 nutzt diese Kombination als Mindest-Einheit für Antwort-Feedback; Lektion = Heavy, Pfad = Peak (Heavy + optional zweiter Impuls).
...
|---|---|---|
| Richtige Antwort (Lektion) | `Success Notification` + `Medium Impact` | gleich |
| Falsche Antwort | `Warning Notification` + `Medium Impact` (nur bei Nutzerursache) | gleich |
| Lektion abgeschlossen | `Success Notification` + `Heavy Impact` | gleich |
| Pfad abgeschlossen + Zertifikat | `Success Notification` + `Heavy Impact` | + zusätzlicher `Medium Impact` (Peak) |
| Orb-Gewinn | `Success Notification` + `Medium Impact` | gleich |
| BYOK-Key erfolgreich validiert | `Success Notification` + `Medium Impact` | gleich |
| Matching – Paar final korrekt verbunden | `Success Notification` + `Medium Impact` – **nur beim finalen Match** | gleich |
| Categorize – Item korrekt zugeordnet | `Success` + `Medium` pro Item; Set-Abschluss: `Success` + `Heavy` | gleich |
| Prompt-Lab-Vergleich: Nutzer-/Validierungsfehler | `Error Notification` + `Medium Impact` | gleich |
| Prompt-Lab-Vergleich: Netzwerk/Provider | `Warning Notification` + `Medium Impact` | gleich |
