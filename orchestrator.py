...
def call_local_model(model_name: str, system_prompt: str, user_prompt: str) -> str:
    ...
    for attempt in range(1, MODEL_CALL_RETRIES + 1):
        try:
            ...
            # Kompatibel mit mehreren ollama-python Versionen:
            # - ältere Versionen: dict response["message"]["content"]
            # - neuere Versionen: ChatResponse-Objekt mit .message.content
            ...
            time.sleep(backoff)
    raise RuntimeError(f"Modell-Call dauerhaft fehlgeschlagen ({model_name}): {last_error}")
...
def _check_import_paths(code: str, file_path: str) -> List[str]:
    ...
    for match in re.finditer(r"from\s+['\"]([^'\"]+)['\"]", code):
        ...
        if spec.startswith("@"):
            issues.append(
                f"Alias-Import '{spec}' ist verboten — nur relative Imports zu existierenden Dateien."
            )
            continue
...
def get_contract_for_target(file_path: str) -> str:
    ...
    if "/shared/theme/" in normalized or "/shared/ui/" in normalized:
        return read_contract("theme.contract.md")
    return ""
...
7. Lehne NUR bei objektiven, nachweisbaren Regelverstößen ab. Keine spekulativen Ablehnungen.
...
# ---------------------------------------------------------------------
def build_file_with_quality_gate(task: Dict[str, object]) -> bool:
...
if __name__ == "__main__":
    ...
    for i, task in enumerate(ordered_task_queue):
        ...
        success = build_file_with_quality_gate(task)
