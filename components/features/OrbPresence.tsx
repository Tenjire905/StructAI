...
type OrbPresenceProps = {
  ...
  showSpeech?: boolean;
  /**
   * Explicit coach line. When set, wins over state-derived speech.
   * Prefer one clear line — Liftoff/Jimbo style, not copy stacks.
   */
  speechKey?: string | null;
  /** Rotates state-fallback lines when speechKey is omitted. */
  speechSeed?: number;
  /** hero = centered coach (onboarding); coach = orb + bubble row */
  layout?: 'coach' | 'hero';
  interaction?: 'none' | 'enter' | 'watch' | 'react';
};

/**
 * Orb presence — local coach via motion + speech bubble (no voiceover).
 * Onboarding uses hero + explicit speechKey so the Orb leads like Jimbo.
 */
          ...
          <Animated.View
            ...
            style={[
              speechStyle,
              {
                ...
                borderWidth: 1,
                maxWidth: tokens.spacing.space8 * 4.5,
