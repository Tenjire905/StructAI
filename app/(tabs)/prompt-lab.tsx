...
function ScoreResult({ score, feedbackKey, comparison, promptText }: ScoreResultProps) {
  ...
  const nextPatternExampleKey = improvementPath
    ? `promptLab.nextPattern.example.${improvementPath.primary}`
    : null;
    ...
    <Animated.View style={animatedStyle}>
      <Card variant="solid">
        <View style={{ gap: tokens.spacing.space4 }}>
          ...
          {/* One next-pattern payoff — no secondary tip, no comparison note list. */}
          <View
            style={{
            ...
            {improvementPath && nextPatternExampleKey ? (
              <>
                <Text
                  style={{
                    color: tokens.colors.text.secondary,
